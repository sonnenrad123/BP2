...
namespace MusicCompetitionBP2.Repositories
{
    public class CompetitorRepository
    {
        ...
        public Common.Models.Competitor Read(long JMBG)
        {
            ...
            return new Common.Models.Competitor(temp.JMBG_SIN, temp.FIRSTNAME_SIN, temp.LASTNAME_SIN, temp.BIRTHDATE_SIN, temp.EMAIL_SIN, temp.PHONE_NO_SIN, new Common.Models.ADDRESS(temp.ADDRESS_SIN.HOME_NUMBER, temp.ADDRESS_SIN.CITY, temp.ADDRESS_SIN.STREET));
        }
        ...
        public IEnumerable<Common.Models.Competitor> ReadAll()
        {
            ...
            dbContext.Singers.AsNoTracking().ToList().ForEach((temp) =>
            {
                if (temp.Type == "Competitor")
                {
                    ret.Add(new Common.Models.Competitor(temp.JMBG_SIN, temp.FIRSTNAME_SIN, temp.LASTNAME_SIN, temp.BIRTHDATE_SIN, temp.EMAIL_SIN, temp.PHONE_NO_SIN, new Common.Models.ADDRESS(temp.ADDRESS_SIN.HOME_NUMBER, temp.ADDRESS_SIN.CITY, temp.ADDRESS_SIN.STREET)));
                }
            });
            return ret;
        }
